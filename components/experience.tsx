...
const experiences = [
  {
    ...
    highlights: [
      "Built CRM handling 10K+ concurrent users with 65% query time reduction through database replica sets and indexing; deployed Chroma vector database with FAISS for semantic property search using vector embeddings",
      "Developed multi-agent LLM system with RAG for semantic search with FastAPI endpoints, automating 80% of real-estate agent tasks and processing 500+ daily requests while implementing rate limiting on token usage that reduced API costs by 40%",
      "Increased search relevance by 31% by experimentation with semantic chunking strategies (sentence-window, overlap heuristics) and embedding model comparison (text-embedding-3-large vs bge-base-en-v1.5)",
    ...
    tags: ["AI", "CRM", "MERN", "LangGraph", "FastAPI", "RAG", "Chroma", "FAISS"],
  },
  {
    ...
    highlights: [
      "Trained a deep learning model for gallbladder cancer detection from 3D gallbladder scans, using 3D Faster R-CNN for region detection and a 3D Swin Transformer for classification",
      "Trained a model on 56 patient CT scans with a 6:1 benign-to-malignant ratio, used focal loss to handle the imbalance, and achieved 84% accuracy and 88% sensitivity",
    ...
    tags: ["Computer Vision", "TensorRT", "Medical AI", "Faster-RCNN", "Swin Transformer"],
  },
  {
